...
const PscData = (props) => {
    ...
    const [psc, setpsc] = useState(props.psc);
    ...
    return <li className="d-flex flex-row justify-content-between list-group-item list-group-item-action w-100">
        <input type="number" className="form-control mx-2" onChange={(e) => setpsc(e.target.value)} defaultValue={props.psc} />
...
export const PscPage = () => {
    ...
    const fetchData = () => {
        axios.get(IP + "/psc").then(response => setData(response.data));
    }
    ...
    return <div className="d-flex flex-column flex-nowrap w-100 mx-5">
        ...
        <ul className="list-group list-group-flush">
            ...
            <li className="d-flex flex-row justify-content-between list-group-item list-group-item-action w-100">
                <input type="number" className="form-control mx-2" onChange={e => setpsc(e.target.value)} value={psc} />
                <button type="button" className="btn btn-primary mx-2" onClick={vytvoritData}>Vytvořit</button>
            </li>
            <li className="d-flex flex-row justify-content-between list-group-item list-group-item-action w-100">
                <h5 className="mx-2">Název</h5>
            </li>
            {data.map((m) => <PscData key={m.id_psc} id_psc={m.id_psc} psc={m.psc} onRefresh={fetchData}/>)}
        </ul>
    </div>
}