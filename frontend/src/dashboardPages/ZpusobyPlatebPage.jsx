...
const ZpusobPlatbyData = (props) => {
    ...
    const upravitData = () => {
        showModal({
            ...
            onConfirm: () => {
                axios.put(IP + "/zpusobyplateb",
                    {
                        id_zpusob_platby: props.id_zpusob_platby,
                        nazev: nazev 
                    }
                ).then(response => {
    ...
    const smazatData = () => {
        showModal({
            ...
            onConfirm: () => {
                axios.delete(
                    IP + "/zpusobyplateb/" + props.id_zpusob_platby 
                ).then(response => {
                    if (response.data.status_code == 1) {
                        showModal({
                            type: 'info',
...
export const ZpusobPlatbyPage = () => {
    ...
    const fetchData = () => {
        axios.get(IP + "/zpusobyplateb").then(response => setData(response.data));
    }
    ...
    return <div className="d-flex flex-column flex-nowrap w-100 mx-5">
        ...
        <ul className="list-group list-group-flush">
            ...
            {data.map((m) => <ZpusobPlatbyData key={m.id_zpusob_platby} id_zpusob_platby={m.id_zpusob_platby} nazev={m.nazev} onRefresh={fetchData}/>)}
        </ul>
    </div>
}